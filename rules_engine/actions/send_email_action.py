...
class SendEmailAction(BaseAction):
    ...
    def _get_data_points(self, event: EVENT_TYPE, rule: Rule, journey_id: Optional[UUID]) -> dict:
        """
        Get the data points to be used in the email template
        """
        data_points: Union[AlertDataPoints, DataPoints]
        context = dict()
        try:
        ...
        match event:
            ...
            case Event():
                ...
                context["component_name"] = data_points.component.name
                context["project_name"] = data_points.project.name
                context["run_expected_start_time"] = getattr(data_points.run, "expected_start_time_formatted", None)
                context["run_expected_end_time"] = getattr(data_points.run, "expected_end_time_formatted", None)
                context["event_timestamp_formatted"] = data_points.event.event_timestamp_formatted
                try:
                    context["task_name"] = data_points.task.name
                except Exception:
                    context["task_name"] = ""
                try:
                    context["run_task_start_time"] = data_points.run_task.start_time_formatted
